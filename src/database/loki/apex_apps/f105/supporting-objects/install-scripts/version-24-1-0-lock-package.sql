create or replace package loki_lock as
...
create or replace package body loki_lock as
  ...
  begin
    select ora_dict_obj_type
     ...
     where loki_locking_type in ( 'DIRECT',
                                  'ESCALATED',
                                  'MIXED' )
  ...
  begin
    ...
    insert into loki_locks (
      ...
      user_id,
      ddl_log_id
    ) values ( io_lock.schema_name,
               io_lock.object_type,
               io_lock.object_name,
               io_lock.user_id,
               io_lock.ddl_log_id ) returning lock_id,
  ...
  procedure handle_ddl_event is
    ...
    l_lock          loki_locks%rowtype;
  begin
    if loki_util.session_user_is_dba() then
      return;
    end if;

    -- Check is user if registered.
    ...
    insert into loki_ddl_logs (
      ...
      event
               ...
               ora_dict_obj_name,
               ora_dict_obj_type,
               ora_sysevent ) returning ddl_log_id into l_ddl_log_id;
