create or replace package body loki_lock as
  ...
  begin
    select ora_dict_obj_type
     ...
     where loki_locking_type in ('DIRECT', 'ESCALATED', 'MIXED')
  ...
  begin
    if i_from = 'ORA_DICT_OBJ_TYPE' then
      select loki_object_type
        into l_loki_object_type
        from loki_object_types
       where ora_dict_obj_type = i_object_type;
    elsif i_from = 'DBA_OBJECTS_OBJECT_TYPE' then
      select loki_object_type
        into l_loki_object_type
        from loki_object_types
       where dba_objects_object_type = i_object_type;
    end if;
  ...
  function get_lock_details (i_ddl in clob) return r_lock_details_type is
    ...
    begin
      return ora_sysevent = 'RENAME' or (
        ora_sysevent = 'ALTER'
        and ora_dict_obj_type in ( 'TABLE',
                                   'INDEX',
                                   'TRIGGER' )
        and regexp_like(
                       i_ddl,
    ...
    procedure escalate_lock is
      ...
      function normalize_name (i_name in varchar2) return varchar2 is
        l_name varchar2(128);
      begin
        l_name := replace(
                         i_name,
    ...
    begin
      if l_lock_details.object_type = 'PACKAGE_SPEC' then
        ...
        if ora_sysevent = 'CREATE' then
          ...
          if regexp_instr(
                         i_ddl,
                         l_index_base_owner_re
             ) > 0 then
            l_base_object_rec.owner := normalize_name(regexp_substr(
                                                                   i_ddl,
                                                                   l_index_full_re,
                                                                   1,
                                                                   1,
          ...
          l_base_object_rec.object_name := normalize_name(regexp_substr(
                                                                       i_ddl,
                                                                       l_index_full_re,
                                                                       1,
                                                                       1,
  ...
  function get_lock(i_lock_id loki_locks.lock_id%type) return loki_locks%rowtype is
    l_lock loki_locks%rowtype;
  begin
    select *
      into l_lock
      from loki_locks
     where lock_id = i_lock_id;
  ...
  procedure create_lock(io_lock in out nocopy loki_locks%rowtype) is
  begin
    loki_util.assert(
  ...
  begin
    ...
    then
      raise_application_error(
        -20000,
        'Loki: Only admins can unlock other users locks'
      );
    end if;
    ...
    update loki_locks_logs
       set released = localtimestamp,
           released_by = loki_util.get_session_user()
     where lock_id = i_lock_id;
  end unlock_lock;

  procedure unlock_locks(i_locks t_lock_user_type) is
  begin
    loki_util.assert(
      i_condition => i_locks is not null or i_locks.count = 0, 
  ...
  begin
    ...
    then
      raise_application_error(
        -20000,
        'Loki: Only admins can transfer other users locks'
      );
    end if;

    unlock_lock(i_lock_id => i_lock_id, i_user_id => l_lock.user_id);
  ...
  begin
    loki_util.assert(
      i_condition => i_lock_ids is not null or i_lock_ids.count = 0, 
      i_message   => 'Loki: Lock IDs are invalid'
    );
  ...
  begin
    ...
    if l_user.user_id is null then
      ...
      raise_application_error(
        -20000,
        'Loki: user is not registered (APEX User: '
        || l_apex_user
        || '; DB User: '
        || user
        || '; DB Proxy User: '
    ...
    for chunk_number in 1..l_sql_count loop
      ...
      if chunk_number = l_sql_count then
        l_sql_chunk := rtrim(l_sql_parts(chunk_number), chr(0));
      end if;

      if length(l_sql_chunk) > 0 then
        dbms_lob.writeappend(l_ddl, length(l_sql_chunk), l_sql_chunk);
      end if;
    end loop;
    ...
    if l_user.user_id = l_lock_user_id then
      if l_lock_details.event_is_rename then
        update loki_locks
           set object_name = l_lock_details.object_name,
  ...
  procedure clear_locks_log(i_log_retention_months pls_integer) is
  begin
    delete from loki_locks_logs
    where released <= CURRENT_TIMESTAMP - interval '1' month;
  end clear_locks_log;

  procedure clear_ddl_log(i_log_retention_months pls_integer) is
  begin
    delete from loki_ddl_logs
    where logged <= CURRENT_TIMESTAMP - interval '1' month;
  end clear_ddl_log;
end loki_lock;
/